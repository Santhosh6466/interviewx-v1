...
export default function UpdateProfile() {
  const { user, setUser, profileCompleted, fetchProfile } = useAuth();
  ...
  const [currentCompany, setCurrentCompany] = useState(null); // { id, name, logoUrl }
  ...
  const currentCompletion = (() => {
    ...
    // 1. Name: 15%
    if (name && name.trim()) score += 15;
    
    // 2. Headline: 10%
    if (headline && headline.trim()) score += 10;
    
    // 3. Bio: 15%
    if (bio && bio.trim()) score += 15;
    
    // 4. Career Status: 10%
    if (careerStatus) score += 10;
    
    // 5. College or Current Company: 15%
    if (careerStatus === 'STUDENT' || careerStatus === 'FRESHER') {
      if (college && college.trim()) score += 15;
    } else {
      if (currentCompanyId || (currentCompany && currentCompany.name)) score += 15;
    }
    
    // 6. Branch or Current Role: 10%
    if (careerStatus === 'STUDENT' || careerStatus === 'FRESHER') {
      if (branch && branch.trim()) score += 10;
    } else {
      if (currentRole && currentRole.trim()) score += 10;
    }
    ...
    // 8. LinkedIn: 10%
    if (linkedinUrl && linkedinUrl.trim()) score += 10;
    
    // 9. GitHub: 10%
    if (githubUrl && githubUrl.trim()) score += 10;
    
    return score;
  })();

  // Company autocomplete search states
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        if (data) {
          setName(data.name || data.fullName || user?.name || '');
          setAvatarSeed(data.avatarSeed || user?.avatarSeed || 'default-avatar');
          setHeadline(data.headline || '');
          setBio(data.bio || '');
          setCareerStatus(data.careerStatus || 'STUDENT');
          setCollege(data.college || '');
          setBranch(data.branch || '');
          setGraduationYear(data.graduationYear || '');
          if (data.currentCompany) {
            setCurrentCompany(data.currentCompany);
            setCurrentCompanyId(data.currentCompany.id || data.currentCompanyId);
            setSearchQuery(data.currentCompany.name || '');
          } else {
        ...
        console.warn('[UpdateProfile] Failed to load existing profile:', err);
        setName(user?.name || '');
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      try {
        ...
        console.error('[UpdateProfile] Company search error:', err);
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update global context profile status
      await fetchProfile();
      
      toast.success('Profile saved successfully!');
      window.location.hash = '#/dashboard';
    } catch (err) {
      console.error('[UpdateProfile] Save failed:', err);
    ...
    return (
      <DashboardLayout activeTab="Profile">
        <div className="flex flex-col items-center justify-center min-h-[400px]">
          <div className="h-8 w-8 animate-spin rounded-full border-4 border-theme-border border-t-theme-inverted"></div>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="flex flex-col gap-8 max-w-[800px] mx-auto w-full fade-in-up">
        ...
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold tracking-tight">
            {profileCompleted === false ? 'Complete Your Profile' : 'Edit Profile'}
          </h1>
          <p className="text-theme-muted text-sm">
            {profileCompleted === false 
              ? 'Please provide your profile details to continue accessing InterviewX.'
              : 'Keep your professional and academic information updated.'
            }
          </p>
        </div>

        {/* Interactive Profile Completion Bar */}
        <div className="premium-card p-5 flex flex-col gap-2 border border-theme-border bg-theme-hover">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-bold text-theme-muted">{currentCompletion}% Completed</span>
          </div>
          {/* Progress Bar */}
          <div className="w-full bg-theme-border rounded-sm h-2 overflow-hidden">
            <div 
              className={`h-full transition-all duration-300 ease-out ${
                currentCompletion === 100 ? 'bg-emerald-500' : 'bg-[#a78b71]'
              }`}
          ...
          <p className="text-xs text-theme-muted mt-1">
            {currentCompletion === 100 
              ...
              : 'Add details like bio, company/college, or social links to reach 100% Strength.'
            }
          </p>
        </div>
        ...
        <form className="flex flex-col gap-8" onSubmit={handleSubmit}>
          ...
          <div className="premium-card flex flex-col gap-6">
            <h2 className="text-lg font-bold pb-4 border-b border-theme-border">Education & Career</h2>
          ...
          <div className="premium-card flex flex-col gap-6">
            <h2 className="text-lg font-bold pb-4 border-b border-theme-border">Social & Coding Profiles</h2>
          ...
          <div className="flex justify-end gap-4">
            {profileCompleted !== false && (
              <a 
                href="#/profile" 
                className="px-6 py-3 bg-transparent border border-theme-border hover:bg-theme-hover rounded-sm text-sm font-bold text-theme-text transition-all"
              >
                Cancel
              </a>
            )}
